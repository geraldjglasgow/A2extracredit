...
public class SelectR {
    ...
    public SelectR(int[] values, int iterations, double micro){
        this.values = values;
    ...
    public void SelectR(int n, int k){
        int low =0; int up = n;
        boolean found = false;
        //ctrl.printArray(a);
    ...
    public void startSelectR() {
        ...
        for (int k = 0; k < values.length; k++) {
            for (int i = 0; i < iterations; i++) {
                ...
                x = System.nanoTime();
                SelectR(a.length-1, rnd.nextInt(a.length));
                y = System.nanoTime();
    ...
    private void getRandom(int start, int end){
        int x;
        if(start != end){ x = rnd.nextInt(end-start) + start;}
        else x = start;
